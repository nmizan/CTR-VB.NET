...
Public Class FrmExDesig

#Region "Global Variables"
    Dim _formName As String = "MaintenanceExecutiveDesignitionDetail"
    Dim opt As SecForm = New SecForm(_formName)

    Dim _formMode As FormTransMode
    'Dim _intSlno As Integer = 0
    Dim _intModno As Integer = 0
    Dim _strExDesig_Code As String = ""
    Dim log_message As String
    Dim _EName As String = ""

    Dim _ExdesigName As String = ""
#End Region
...
#Region "User defined Codes"
    ...
    Private Sub DisableFields()
        txtExDesigCode.ReadOnly = True
        txtExDesigName.ReadOnly = True
    ...
    Private Sub EnableFields()
        txtExDesigCode.ReadOnly = False
        txtExDesigName.ReadOnly = False
    ...
    Private Sub ClearFields()
        txtExDesigCode.Clear()
        txtExDesigName.Clear()
    ...
    Private Function SaveData() As TransState
        ...
        If _formMode = FormTransMode.Add Then  ' OK
            ...
            strSql = "Insert Into FIU_EXECUTIVE_DESIG(EXE_DESIG_CODE, EXE_DESIG_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED,  STATUS)" & _
                     "Values(@P_EXE_DESIG_CODE, @P_EXE_DESIG_NAME,@P_Inserted_On,@P_Modified_On,1,@P_Input_By,getdate(),0,'U')"

            Dim commProc As DbCommand = db.GetSqlStringCommand(strSql)

            commProc.Parameters.Clear()
            db.AddInParameter(commProc, "@P_EXE_DESIG_CODE", DbType.String, txtExDesigCode.Text.Trim())
            db.AddInParameter(commProc, "@P_EXE_DESIG_NAME", DbType.String, txtExDesigName.Text.Trim())
            ...
            Else
                ...
                '_intSlno = intSlno
                _strExDesig_Code = txtExDesigCode.Text.Trim()

                _intModno = 1

                log_message = " Added : Designition Code : " + txtExDesigCode.Text.Trim() + " " + " Designition Name : " + txtExDesigName.Text.Trim()
        ...
        ElseIf _formMode = FormTransMode.Update Then
            ...
            Using conn As DbConnection = db.CreateConnection()
                ...
                db.ExecuteNonQuery(trans, CommandType.Text, "delete FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "' and IS_AUTHORIZED=0")
                Dim ds As New DataSet


                strSql = "select isnull(max(MODNO),0)+1 maxmodno from FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "'"


                intModno = db.ExecuteDataSet(trans, CommandType.Text, strSql).Tables(0).Rows(0)(0)

                strSql = "Insert Into FIU_EXECUTIVE_DESIG(EXE_DESIG_CODE, EXE_DESIG_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED, STATUS)" & _
                     "Values(@P_EXE_DESIG_CODE, @P_EXE_DESIG_NAME,@P_Inserted_On,@P_Modified_On," & intModno.ToString() & ",@P_Input_By,getdate(),0,'U')"
                ...
                Else
                    ...
                    '------------Mizan Work (17-04-16)---------
                    If _EName <> txtExDesigName.Text.Trim() Then

                        log_message = " Updated : Designition Code : " + txtExDesigCode.Text.Trim() + "." + " " + " Designition Name : " + _EName + " " + " To " + " " + txtExDesigName.Text.Trim()
                        Logger.system_log(log_message)
                    Else
                        log_message = " Updated : Designition Code : " + txtExDesigCode.Text.Trim()
                        Logger.system_log(log_message)
                    End If
                    '------------Mizan Work (17-04-16)---------
    ...
    '------------Mizan Work (17-04-16)---------
    Private Sub LoadExDesigDataForAuth(ByVal strExDesigCode As String)
        ...
        Try
            ...
            ds = db.ExecuteDataSet(CommandType.Text, "Select * From FIU_EXECUTIVE_DESIG Where EXE_DESIG_CODE='" & strExDesigCode & "' and STATUS = 'L' ")
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                _strExDesig_Code = strExDesigCode


                _formMode = FormTransMode.Update

                txtExDesigCode.Text = ds.Tables(0).Rows(0)("EXE_DESIG_CODE").ToString()

                'txtExDesigName.Text = ds.Tables(0).Rows(0)("EXE_DESIG_NAME").ToString()
                _ExdesigName = ds.Tables(0).Rows(0)("EXE_DESIG_NAME").ToString()
    ...
    Private Sub LoadExDesigData(ByVal strExDesigCode As String, ByVal intMod As Integer)
        ...
        Try
            ...
            ds = db.ExecuteDataSet(CommandType.Text, "Select * From FIU_EXECUTIVE_DESIG Where EXE_DESIG_CODE='" & strExDesigCode & "' and MODNO=" & intMod.ToString())
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                txtExDesigName.Text = ds.Tables(0).Rows(0)("EXE_DESIG_NAME").ToString()
                _EName = ds.Tables(0).Rows(0)("EXE_DESIG_NAME").ToString()
                ...
                lblVerTot.Text = db.ExecuteDataSet(CommandType.Text, "Select Count(MODNO) From FIU_EXECUTIVE_DESIG Where EXE_DESIG_CODE='" & strExDesigCode & "'").Tables(0).Rows(0)(0).ToString()
    ...
    Public Sub New(ByVal strExDesigCode As String, ByVal intMod As Integer)

        ' This call is required by the Windows Form Designer.
        InitializeComponent()
        LoadExDesigData(strExDesigCode, intMod)
    ...
    Private Function AuthorizeData() As TransState
        ...
        If _intModno > 1 Then
            ...
            LoadExDesigDataForAuth(_strExDesig_Code)
            ...
            Using conn As DbConnection = db.CreateConnection()
                ...
                strSql = "select IS_AUTHORIZED,STATUS from FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & _intModno.ToString()
                ...
                If ds.Tables(0).Rows(0)("IS_AUTHORIZED") = 0 Then
                    ...
                    If ds.Tables(0).Rows(0)("STATUS") = "U" Then
                        strSql = "update FIU_EXECUTIVE_DESIG set AUTH_BY='" & CommonAppSet.User.ToString().Trim() & _
                        "', AUTH_DATETIME=getdate(), IS_AUTHORIZED=1, STATUS = 'L' " & _
                        " where  EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & _intModno.ToString()
                    ...
                    ElseIf result > 0 Then
                        ...
                        If _intModno > 1 Then
                            ...
                            strSql = "update FIU_EXECUTIVE_DESIG set STATUS = 'C' " & _
                                " where EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & (_intModno - 1).ToString() & _
                                " and STATUS ='D'"

                            db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                            'if previous modification status is L(Deleted) then make it O(Open)
                            strSql = "update FIU_EXECUTIVE_DESIG set STATUS = 'O' " & _
                                " where EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & (_intModno - 1).ToString() & _
                                " and STATUS ='L'"
                        ...
                        If _ExdesigName <> _EName Then
                            If _ExdesigName = "" Then
                                log_message = " Authorized : Executive Designation Code : " + txtExDesigCode.Text.Trim() + "." + " " + " Designation Name : " + _EName
                            Else
                                log_message = " Authorized : Executive Designation Code : " + txtExDesigCode.Text.Trim() + "." + " " + " Designation Name : " + _ExdesigName + " " + " To " + " " + _EName
                            End If

                            Logger.system_log(log_message)
                        Else
                            log_message = " Authorized : Executive Designation Code : " + txtExDesigCode.Text.Trim()
                            Logger.system_log(log_message)
                        End If
        ...
        '    'log_message = "Authorized Designition Code " + txtExDesigCode.Text.Trim() + " Designition Name " + txtExDesigName.Text.Trim()
        '    'Logger.system_log(log_message)
        ...
        Using conn As DbConnection = db.CreateConnection()
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows(0)(0) = False Then 'if not authorized
                    ...
                    strSql = "delete FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & _intModno.ToString() & " and IS_AUTHORIZED=0"
                ...
                ElseIf ds.Tables(0).Rows(0)(0) = True Then 'if authorized
                    ...
                    If ds.Tables(0).Rows(0)("STATUS") = "L" Then 'if this is the last modified data
                        ...
                        strSql = "delete FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "'  and IS_AUTHORIZED=0"

                        db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                        strSql = "select * from FIU_EXECUTIVE_DESIG where EXE_DESIG_CODE='" & _strExDesig_Code & "' and MODNO=" & _intModno.ToString()
                        ...
                        strSql = "Insert Into FIU_EXECUTIVE_DESIG(EXE_DESIG_CODE, EXE_DESIG_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED,  STATUS)" & _
                     "Values(@P_EXE_DESIG_CODE, @P_EXE_DESIG_NAME, @P_Inserted_On,@P_Modified_On," & intModno.ToString() & ",@P_Input_By,getdate(),0,'D')"
                        ...
                        db.AddInParameter(commProc, "@P_EXE_DESIG_CODE", DbType.String, dsKeeper.Tables(0).Rows(0)("EXE_DESIG_CODE"))
                        db.AddInParameter(commProc, "@P_EXE_DESIG_NAME", DbType.String, dsKeeper.Tables(0).Rows(0)("EXE_DESIG_NAME"))
            ...
            log_message = "Delete Designition Code " + txtExDesigCode.Text.Trim()
            Logger.system_log(log_message)
...
#End Region
    ...
    Private Sub FrmExDesig_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        If txtExDesigCode.Enabled = True Then
            txtExDesigCode.Focus()
        End If
            ...
            If MessageBox.Show("Do you really want to Save?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                ...
                If CheckValidData() Then  ' Ok
                    ...
                    If tState = TransState.Add Then
                        ...
                        LoadExDesigData(_strExDesig_Code, _intModno)
    ...
    Private Sub btnNextVer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNextVer.Click
        ...
        If _intModno = 0 Then
            ...
            LoadExDesigData(strExDesig_Code, intModno)
        End If
    End Sub
End Class