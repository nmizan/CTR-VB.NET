...
Public Class FrmBankGroup

#Region "Global Variables"

    Dim _formName As String = "MaintenanceBankGroupDetail"
...
#Region "User defined Codes"
    ...
    Private Function SaveData() As TransState
        ...
        If _formMode = FormTransMode.Add Then  ' OK
            ...
            strSql = "Insert Into FIU_BANKGROUP(GROUP_CODE, GROUP_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED,  STATUS)" & _
                     "Values(@P_GROUP_CODE, @P_GROUP_NAME,@P_Inserted_On,@P_Modified_On,1,@P_Input_By,getdate(),0,'U')"

            Dim commProc As DbCommand = db.GetSqlStringCommand(strSql)

            commProc.Parameters.Clear()
            db.AddInParameter(commProc, "@P_GROUP_CODE", DbType.String, txtGroupCode.Text.Trim())
            db.AddInParameter(commProc, "@P_GROUP_NAME", DbType.String, txtGroupName.Text.Trim())
            ...
            Else
                ...
                '_intSlno = intSlno
                _strGroup_Code = txtGroupCode.Text.Trim()

                _intModno = 1
            End If

            log_message = "Group Code " + txtGroupCode.Text.Trim() + " Group Name " + txtGroupName.Text.Trim() + " Added"
        ...
        ElseIf _formMode = FormTransMode.Update Then
            ...
            Using conn As DbConnection = db.CreateConnection()
                ...
                db.ExecuteNonQuery(trans, CommandType.Text, "delete FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "' and IS_AUTHORIZED=0")
                Dim ds As New DataSet


                strSql = "select isnull(max(MODNO),0)+1 maxmodno from FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "'"


                intModno = db.ExecuteDataSet(trans, CommandType.Text, strSql).Tables(0).Rows(0)(0)

                strSql = "Insert Into FIU_BANKGROUP(GROUP_CODE, GROUP_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED, STATUS)" & _
                     "Values(@P_GROUP_CODE, @P_GROUP_NAME,@P_Inserted_On,@P_Modified_On," & intModno.ToString() & ",@P_Input_By,getdate(),0,'U')"
                ...
                log_message = "Update Group Code " + txtGroupCode.Text.Trim() + " Group Name " + txtGroupName.Text.Trim()
                Logger.system_log(log_message)
    ...
    Private Sub LoadBankGroupData(ByVal strBankGroupCode As String, ByVal intMod As Integer)
        ...
        Try
            ...
            ds = db.ExecuteDataSet(CommandType.Text, "Select * From FIU_BANKGROUP Where GROUP_CODE='" & strBankGroupCode & "' and MODNO=" & intMod.ToString())
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                _strGroup_Code = strBankGroupCode
                _intModno = intMod
                ...
                txtGroupCode.Text = ds.Tables(0).Rows(0)("GROUP_CODE").ToString()
                ...
                lblVerTot.Text = db.ExecuteDataSet(CommandType.Text, "Select Count(MODNO) From FIU_BANKGROUP Where GROUP_CODE='" & strBankGroupCode & "'").Tables(0).Rows(0)(0).ToString()
    ...
    Public Sub New(ByVal strBankGroupCode As String, ByVal intMod As Integer)

        ' This call is required by the Windows Form Designer.
        InitializeComponent()
        LoadBankGroupData(strBankGroupCode, intMod)
    ...
    Private Function AuthorizeData() As TransState
        ...
        Using conn As DbConnection = db.CreateConnection()
            ...
            strSql = "select IS_AUTHORIZED,STATUS from FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & _intModno.ToString()
            ...
            If ds.Tables(0).Rows(0)("IS_AUTHORIZED") = 0 Then
                ...
                If ds.Tables(0).Rows(0)("STATUS") = "U" Then
                    strSql = "update FIU_BANKGROUP set AUTH_BY='" & CommonAppSet.User.ToString().Trim() & _
                    "', AUTH_DATETIME=getdate(), IS_AUTHORIZED=1, STATUS = 'L' " & _
                    " where  GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & _intModno.ToString()
                ...
                ElseIf result > 0 Then
                    ...
                    If _intModno > 1 Then
                        ...
                        strSql = "update FIU_BANKGROUP set STATUS = 'C' " & _
                            " where GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & (_intModno - 1).ToString() & _
                            " and STATUS ='D'"

                        db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                        'if previous modification status is L(Deleted) then make it O(Open)
                        strSql = "update FIU_BANKGROUP set STATUS = 'O' " & _
                            " where GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & (_intModno - 1).ToString() & _
                            " and STATUS ='L'"
            ...
            log_message = "Authorized Group Code " + _strGroup_Code.ToString()
            Logger.system_log(log_message)
        ...
        Using conn As DbConnection = db.CreateConnection()
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows(0)(0) = False Then 'if not authorized
                    ...
                    strSql = "delete FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & _intModno.ToString() & " and IS_AUTHORIZED=0"
                ...
                ElseIf ds.Tables(0).Rows(0)(0) = True Then 'if authorized
                    ...
                    If ds.Tables(0).Rows(0)("STATUS") = "L" Then 'if this is the last modified data
                        ...
                        strSql = "delete FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "'  and IS_AUTHORIZED=0"

                        db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                        strSql = "select * from FIU_BANKGROUP where GROUP_CODE='" & _strGroup_Code & "' and MODNO=" & _intModno.ToString()
                        ...
                        strSql = "Insert Into FIU_BANKGROUP(GROUP_CODE, GROUP_NAME, INSERTED_ON,  MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED,  STATUS)" & _
                     "Values(@P_GROUP_CODE, @P_GROUP_NAME, @P_Inserted_On,@P_Modified_On," & intModno.ToString() & ",@P_Input_By,getdate(),0,'D')"
                        ...
                        db.AddInParameter(commProc, "@P_GROUP_CODE", DbType.String, dsKeeper.Tables(0).Rows(0)("GROUP_CODE"))
                        db.AddInParameter(commProc, "@P_GROUP_NAME", DbType.String, dsKeeper.Tables(0).Rows(0)("GROUP_NAME"))
            ...
            log_message = "Delete Group Code " + _strGroup_Code.ToString()
            Logger.system_log(log_message)
...
#End Region
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        If txtGroupCode.Enabled = True Then
            txtGroupCode.Focus()
        End If
            ...
            If MessageBox.Show("Do you really want to Save?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                ...
                If CheckValidData() Then  ' Ok
                    ...
                    If tState = TransState.Add Then
                        ...
                        LoadBankGroupData(_strGroup_Code, _intModno)
    ...
    Private Sub btnNextVer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNextVer.Click
        ...
        If _intModno = 0 Then
            ...
            LoadBankGroupData(strBankGroup_Code, intModno)
        End If
    End Sub
