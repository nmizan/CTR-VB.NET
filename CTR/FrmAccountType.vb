...
#Region "Global Variables"
    Dim _formName As String = "MaintenanceAccountTypeDetail"
    Dim opt As SecForm = New SecForm(_formName)

    Dim _formMode As FormTransMode
    'Dim _intSlno As Integer = 0
    Dim _strAcTypeCode As String = ""
...
#Region "User defined Codes"
    ...
    Private Function SaveData() As TransState
        ...
        If _formMode = FormTransMode.Add Then  ' OK
            ...
            strSql = "Insert Into FIU_ACCOUNT_TYPES(ACTYPECODE, ACDEFINITION, INSERTED_ON, MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED, STATUS) " & _
                     " Values(@P_ACTYPECODE, @P_ACDEFINITION, @P_INSERTED_ON, @P_MODIFIED_ON, 1, @P_INPUT_BY, getdate(), 0, 'U')"


            Dim commProc As DbCommand = db.GetSqlStringCommand(strSql)

            commProc.Parameters.Clear()
            db.AddInParameter(commProc, "@P_ACTYPECODE", DbType.String, txtId.Text.Trim())
            db.AddInParameter(commProc, "@P_ACDEFINITION", DbType.String, txtName.Text.Trim())
            ...
            Else
                ...
                _strAcTypeCode = txtId.Text.Trim()

                _intModno = 1

                log_message = " Added : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + txtName.Text.Trim()
        ...
        ElseIf _formMode = FormTransMode.Update Then
            ...
            Using conn As DbConnection = db.CreateConnection()
                ...
                db.ExecuteNonQuery(trans, CommandType.Text, "delete FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "' and IS_AUTHORIZED=0")
                Dim ds As New DataSet


                strSql = "select isnull(max(MODNO),0)+1 maxmodno from FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "'"


                intModno = db.ExecuteDataSet(trans, CommandType.Text, strSql).Tables(0).Rows(0)(0)

                strSql = "Insert Into FIU_ACCOUNT_TYPES(ACTYPECODE, ACDEFINITION,  INSERTED_ON, MODIFIED_ON, MODNO, INPUT_BY, INPUT_DATETIME, IS_AUTHORIZED, STATUS) " & _
                     " Values(@P_ACTYPECODE, @P_ACDEFINITION," & _
                ...
                Else
                    ...
                    If _AcName <> txtName.Text.Trim() Then

                        log_message = " Updated : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + _AcName + " " + " To " + " " + txtName.Text.ToString()
                        Logger.system_log(log_message)
                    Else
                        log_message = "Updated : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + txtName.Text.Trim()
                        Logger.system_log(log_message)
                    End If
                    '----------Mizan Work (10-04-2016)---------------
    ...
    Private Function AuthorizeData() As TransState
        ...
        If _intModno > 1 Then
            ...
            LoadAccTypeDataForAuth(_strAcTypeCode)
            ...
            Using conn As DbConnection = db.CreateConnection()
                ...
                strSql = "select IS_AUTHORIZED,STATUS from FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & _intModno.ToString()
                ...
                If ds.Tables(0).Rows(0)("IS_AUTHORIZED") = 0 Then
                    ...
                    If ds.Tables(0).Rows(0)("STATUS") = "U" Then
                        strSql = "update FIU_ACCOUNT_TYPES set AUTH_BY='" & CommonAppSet.User.ToString().Trim() & _
                        "', AUTH_DATETIME=getdate(), IS_AUTHORIZED=1, STATUS = 'L' " & _
                        " where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & _intModno.ToString()
                    ...
                    ElseIf result > 0 Then
                        ...
                        If _intModno > 1 Then
                            ...
                            strSql = "update FIU_ACCOUNT_TYPES set STATUS = 'C' " & _
                                " where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & (_intModno - 1).ToString() & _
                                " and STATUS ='D'"

                            db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                            'if previous modification status is L(Deleted) then make it O(Open)
                            strSql = "update FIU_ACCOUNT_TYPES set STATUS = 'O' " & _
                                " where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & (_intModno - 1).ToString() & _
                                " and STATUS ='L'"
                        ...
                        If _AccountName <> _AcName Then
                            If _AccountName = "" Then
                                log_message = " Authorized : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + _AcName
                            Else
                                log_message = " Authorized : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + _AccountName + " " + " To " + " " + _AcName
                            End If

                            Logger.system_log(log_message)
                        Else

                            log_message = "Authorized : Account Type Code : " + txtId.Text.Trim() + "." + " " + " Account Type Name : " + txtName.Text.Trim()
                            Logger.system_log(log_message)
    ...
    '--------------Mizan Work (13-04-16)---------------------
    Private Sub LoadAccTypeDataForAuth(ByVal strAccTypeCd As String)
        ...
        Try
            ...
            ds = db.ExecuteDataSet(CommandType.Text, "Select * From FIU_ACCOUNT_TYPES Where ACTYPECODE='" & strAccTypeCd & "' and STATUS= 'L' ")

            If ds.Tables(0).Rows.Count > 0 Then

                _strAcTypeCode = ds.Tables(0).Rows(0)("ACTYPECODE").ToString()

                _formMode = FormTransMode.Update

                txtId.Text = _strAcTypeCode
                'txtName.Text = ds.Tables(0).Rows(0)("ACDEFINITION").ToString()
                _AccountName = ds.Tables(0).Rows(0)("ACDEFINITION").ToString()
    ...
    Private Sub LoadAccTypeData(ByVal strAccTypeCd As String, ByVal intMod As Integer)
        ...
        Try
            ...
            ds = db.ExecuteDataSet(CommandType.Text, "Select * From FIU_ACCOUNT_TYPES Where ACTYPECODE='" & strAccTypeCd & "' and MODNO=" & intMod.ToString())
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                txtId.Text = _strAcTypeCode
                txtName.Text = ds.Tables(0).Rows(0)("ACDEFINITION").ToString()
                _AcName = ds.Tables(0).Rows(0)("ACDEFINITION").ToString()
                ...
                lblVerTot.Text = db.ExecuteDataSet(CommandType.Text, "Select Count(ModNo) From FIU_ACCOUNT_TYPES Where ACTYPECODE='" & strAccTypeCd & "'").Tables(0).Rows(0)(0).ToString()
        ...
        Using conn As DbConnection = db.CreateConnection()
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows(0)(0) = False Then 'if not authorized
                    ...
                    strSql = "delete FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & _intModno.ToString() & " and IS_AUTHORIZED=0"
                ...
                ElseIf ds.Tables(0).Rows(0)(0) = True Then 'if authorized
                    ...
                    If ds.Tables(0).Rows(0)("STATUS") = "L" Then 'if this is the last modified data
                        ...
                        strSql = "delete FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "' and IS_AUTHORIZED=0"

                        db.ExecuteNonQuery(trans, CommandType.Text, strSql)

                        strSql = "select * from FIU_ACCOUNT_TYPES where ACTYPECODE='" & _strAcTypeCode & "' and MODNO=" & _intModno.ToString()
                        ...
                        strSql = "Insert Into FIU_ACCOUNT_TYPES(ACTYPECODE, ACDEFINITION," & _
                     ...
                     "@P_Inserted_On,@P_Modified_On," & intModno.ToString() & ",@P_Input_By,getdate(),0,'D')"
                        ...
                        commProc.Parameters.Clear()
                        db.AddInParameter(commProc, "@P_ACTYPECODE", DbType.String, _strAcTypeCode)
                        db.AddInParameter(commProc, "@P_ACDEFINITION", DbType.String, dsKeeper.Tables(0).Rows(0)("ACDEFINITION"))
            ...
            log_message = "Delete Account Type Code " + txtId.Text.Trim()
            Logger.system_log(log_message)
...
#End Region
    ...
    Private Sub FrmAccountTypeDet_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If _intModno > 0 Then
            ...
            LoadAccTypeData(_strAcTypeCode, _intModno)
        End If
    ...
    Private Sub btnPrevVer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrevVer.Click
        If _intModno - 1 > 0 Then
            LoadAccTypeData(_strAcTypeCode, _intModno - 1)
    ...
    Private Sub btnNextVer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNextVer.Click
        ...
        Dim strAccTypeCode As String = _strAcTypeCode
        ...
        If _intModno = 0 Then
            ...
            LoadAccTypeData(strAccTypeCode, intModno)
        End If
